class Solution {
    public List<Integer> findSubstring(String s, String[] words) {
        ...
        HashMap<String,Integer> word_frq = new HashMap<>();
        List<Integer> ans = new ArrayList<>();

        // Map store the frequency of every word in words[]

        for (String str : words) {
            word_frq.put(str, 1 + word_frq.getOrDefault(str, 0));
        }
        ...
        for (int i=0;i<wordlen;i++) {
            HashMap<String,Integer> frq=new HashMap<>();  // count frequency of words inside the window
